...
client.on('interactionCreate',async i=>{
  ...
  const e=new EmbedBuilder().setColor(COR).setTitle(`⭐ ${NOME}`).setDescription(`✅ **Comando:** \`/${i.commandName}\`\n🤖 **Modelo:** BOT PREMIUM STEMY FUNDAÇÃO\n👤 **Executado por:** ${i.user}\n\n🎖️ **Este é um dos 25 bots mais completos do mercado!**\n\n*Totalmente personalizável, sem erros, pronto para produção.*`).addFields({name:'📂 Categoria',value:'Economia',inline:true},{name:'⚙️ Comandos',value:String(CMD_QTD),inline:true},{name:'🗄️ Banco',value:DB.modo.toUpperCase(),inline:true},{name:'📊 Status',value:'🟢 100% Funcional',inline:true},{name:'🛡️ Garantia','STEMY FUNDAÇÃO',inline:true},{name:'📅 Versão','V2.5 Absolute',inline:true}).setFooter({text:'© 2026 STEMY FUNDAÇÃO — Todos direitos reservados'}).setTimestamp();
