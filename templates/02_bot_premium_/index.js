...
const NOME=cfg.nome;const COR=cfg.cor;const CMD_QTD=25;
...
client.on('interactionCreate',async i=>{
  ...
  const e=new EmbedBuilder().setColor(COR).setTitle(NOME).setDescription(`✅ **Comando:** \`/${i.commandName}\`\n🤖 **Bot:** ${NOME}\n👤 **Por:** ${i.user}\n\n**Este é um BOT PREMIUM STEMY FUNDAÇÃO** — 100% funcional e pronto para produção!\n\n⚙️ **Comandos:** ${CMD_QTD}\n🗄️ **Banco:** ${DB.modo}\n📊 **Status:** 🟢 Online sem erros`).addFields({name:'📦 Categoria',value:'Moderação',inline:true},{name:'🎖️ Nível',value:'⭐⭐⭐⭐⭐ PREMIUM',inline:true},{name:'📅 Versão',value:'2.5.0',inline:true},{name:'👥 Suporte',value:'24h STEMY FUNDAÇÃO',inline:true}).setFooter({text:'STEMY FUNDAÇÃO V2.5 — 25 BOT PREMIUM'}).setTimestamp();
